...
class Ui_recognapiform(object):
    def setupUi(self, recognapiform):
        ...
        size_policy.setHeightForWidth(recognapiform.sizePolicy().hasHeightForWidth())
        recognapiform.setSizePolicy(size_policy)
    ...
    def retranslateUi(self, recognapiform):
        ...
        if defaulelang == "zh":
            tips = """
请求发送：以二进制形式发送键名为 audio 的 wav 格式音频数据，采样率为 16k、通道为 1

如果填写了密码值，则附加到 api_url 之后发送，api_url?sk=填写的 sk 值
...
res={
    "code":0,
    "data":[
        {"text":"字幕文字","time":"00:00:01,000 --> 00:00:06,500"},
        {"text":"字幕文字","time":"00:00:06,900 --> 00:00:12,200"}
    ]
}
"""
        elif defaulelang == "vi":
            tips = """
Yêu cầu gửi đi: gửi dữ liệu âm thanh WAV dưới dạng nhị phân với khóa `audio`, tần số lấy mẫu 16k, 1 kênh.

Nếu có điền Password/Token, giá trị này sẽ được nối sau `api_url`, ví dụ: `api_url?sk=gia_tri_sk`

requests.post(api_url, files={"audio": open(audio_file, 'rb')})

Khi thất bại, API trả về:
res={
    "code":1,
    "msg":"Lý do lỗi"
}

Khi thành công, API trả về:
res={
    "code":0,
    "data":[
        {"text":"Nội dung phụ đề","time":"00:00:01,000 --> 00:00:06,500"},
        {"text":"Nội dung phụ đề","time":"00:00:06,900 --> 00:00:12,200"}
    ]
}
"""
        else:
...
res={
    "code":0,
    "data":[
        {"text":"Subtitle text","time":"00:00:01,000 --> 00:00:06,500"},
        {"text":"Subtitle text","time":"00:00:06,900 --> 00:00:12,200"}
    ]
}
"""
