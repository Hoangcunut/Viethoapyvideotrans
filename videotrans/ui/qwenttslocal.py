...
class Ui_qwenttslocal(object):
    def setupUi(self, qwenttslocal):
        ...
        size_policy.setHeightForWidth(qwenttslocal.sizePolicy().hasHeightForWidth())
        qwenttslocal.setSizePolicy(size_policy)
        ...
        prompt_label = "Từ gợi ý giọng đọc" if defaulelang == "vi" else ("自定义音色提示词" if defaulelang == "zh" else "Custom Voice Prompt")
        h1_prompt.addWidget(QLabel(prompt_label))
    ...
    def retranslateUi(self, qwenttslocal):
        if defaulelang == "zh":
            tips = """
参考音频填写：
每行都由 # 符号分割为两部分，第一部分是 wav 音频名称，第二部分是该音频对应的文字内容。
wav 音频最佳时长 3-10s，音频必须存放在本软件目录下的 f5-tts 文件夹内，然后在此填写文件名称

参考音频填写示例：
1.wav#你好啊亲爱的朋友
2.wav#你好啊朋友们

# prompt 填写提示词，在使用模型内置音色时（Vivian、Sohee 等），可作为发音指导词，例如请使用愤怒的声音
"""
        elif defaulelang == "vi":
            tips = """
Cách điền âm thanh tham chiếu:
Mỗi dòng được tách làm 2 phần bằng ký tự `#`. Phần thứ nhất là tên tệp WAV, phần thứ hai là nội dung văn bản tương ứng với tệp âm thanh đó.
Độ dài tối ưu của WAV là 3-10 giây. Tệp âm thanh phải nằm trong thư mục `f5-tts` bên trong thư mục phần mềm, sau đó điền tên tệp tại đây.

Ví dụ:
1.wav#Xin chào người bạn thân mến
2.wav#Xin chào các bạn

# prompt Điền câu gợi ý. Khi dùng các giọng tích hợp sẵn của mô hình (Vivian, Sohee, ...), nó có thể đóng vai trò hướng dẫn cách phát âm, ví dụ: hãy dùng giọng tức giận.
"""
        else:
            tips = """
